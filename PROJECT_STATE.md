...
- [ ] מילוי פרטי המפעיל במסמכים המשפטיים (ראו "Placeholders פתוחים" למטה)
- [ ] אפשרות: ביטול/חזרה (undo)
- [ ] אפשרות: PDF מוגן בסיסמת פתיחה (כרגע מוצגת הודעה)

## Placeholders פתוחים במסמכים המשפטיים
`[OPERATOR_NAME]`, `[CONTACT_EMAIL]`, `[PHONE_NUMBER]`, `[JURISDICTION_CITY]`, `[ACCESSIBILITY_COORDINATOR_NAME]`, `[RESPONSE_TIME_DAYS]`, `[ACCESSIBILITY_AUDIT_DATE]` — מסומנים בצהוב בעמוד המשפטי עד שימולאו.
...
- בכל שינוי בקבצי האפליקציה מעלים את `VERSION` ב-`sw.js` (כרגע `easypen-v4`).
