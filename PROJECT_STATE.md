...
- [x] תיקון: SW שמר ב-precache עותק ישן של style.css ממטמון ה-HTTP (`cache: 'reload'`), קישורי תחתית כרשת 2×2, שוליים רחבים יותר במסמכים
...
- בכל שינוי בקבצי האפליקציה מעלים את `VERSION` ב-`sw.js` (כרגע `easypen-v5`).
