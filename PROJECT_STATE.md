...
- [x] תיקון: SW שמר ב-precache עותק ישן של style.css ממטמון ה-HTTP (`cache: 'reload'`), קישורי תחתית כקישורי טקסט ממורכזים ב-2 שורות של 2, שוליים רחבים יותר במסמכים
...
- בכל שינוי בקבצי האפליקציה מעלים את `VERSION` ב-`sw.js` (כרגע `easypen-v6`).
